...
function stopAutoRefresh() {
    if (autoRefreshInterval) {
        ...
        showSuccessNotification("Auto-refresh stopped");
    }
}
...
// Update all charts incrementally
function updateCharts(data) {
    try {
        if (data.bar_chart_data && churnChart) {
            ...
            churnChart.update('none'); // Use 'none' to avoid animation flicker
        }
        ...
        showErrorNotification('Some charts could not be updated.');
    }
}
...
// Calculate trendline for line chart
...
function initializeDataTable() {
    try {
        ...
        showErrorNotification('Failed to initialize data table.');
    }
}

// Refresh all data without blinking
function refreshData() {
    return Promise.all([
        ...
        new Promise(resolve => {
            if (dataTable) {
                dataTable.ajax.reload(resolve, false); // False prevents resetting pagination
            } else {
                resolve();
            }
        })
...
function saveFilterSettings() {
    try {
        ...
        refreshData(); // Refresh data when filters change
    } catch (error) {
...
// Show/hide loading overlay (minimized to avoid flicker)
function showLoading() {
    const overlay = document.getElementById('loadingOverlay');
    if (overlay) overlay.style.opacity = '0.5'; // Subtle loading indication
}
